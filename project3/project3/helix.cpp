...
bool helix::Draw(const glm::mat4 & projection, glm::mat4 modelview, const glm::ivec2 & size)
{
	int i;
	for(i=0; i<windings; i++)
	{
		if(!Super_lattice::Draw(projection, glm::translate(modelview, glm::vec3(0.0f, 0.0f, float(pitch)*float(i))),size))
		{
			std::cerr << "helix::Draw failed" << std::endl;
			return false;
		}
	}
	return true;
